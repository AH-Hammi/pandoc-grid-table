...
// Get the table range
export function get_table_range(
	doc: vscode.TextDocument,
	cur_selection: vscode.Selection,
...
function find_next_cell_in_current_line(
	line_text: string,
	current_col: number,
...
function find_previous_cell_in_current_line(
	line_text: string,
	current_col: number,
): [number, number] | undefined {
	// check char for char until we find a "|"
...
export function get_next_cell_range(
	...
	while (true) {
		...
		start_end = find_next_cell_in_current_line(
			doc.lineAt(current_line).text,
			first_char,
		);
...
export function get_previous_cell_range(
	...
	while (true) {
		// check if there is another cell in the current line
		start_end = find_previous_cell_in_current_line(
			doc.lineAt(current_line).text,
			first_char,
		);
		// if there is another cell in the current line
