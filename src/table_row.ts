...
export class TableRow {
	...
	get_initial_column_widths(): Array<number> {
		const column_widths = [];
		for (let i = 0; i < this._cells.length; i++) {
			column_widths.push(this._cells[i].initial_cell_length);
		}
	...
	get_formatted_row(lengths: Array<number>, add_back_separator = true): string {
		...
		if (lengths.length !== this._cells.length) {
			throw new Error(
				"The length of the lengths array is not the same as the number of cells.",
			);
		}
		...
		formatted_row += this._cells[last_index].get_formatted_cell(
			lengths[last_index],
			add_back_separator,
		);
